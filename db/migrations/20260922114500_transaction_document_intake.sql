-- Create a review transaction when a document is uploaded from Transactions.
-- Existing high-confidence same-currency matches are reused to avoid duplicates.
...
begin
  ...
  if v_kind is null or v_kind not in ('receipt','purchase_invoice','sales_invoice') then
    raise exception 'This document type cannot create a transaction automatically';
  end if;
  ...
  if v_direction is null or v_direction not in ('income','expense') then
    v_direction:=case when v_kind='sales_invoice' then 'income' else 'expense' end;
  end if;
  ...
  if v_counterparty is null then
    v_counterparty:=case
      when v_kind='sales_invoice' then nullif(trim(v_doc.extracted_data->>'customer_name'),'')
      else nullif(trim(v_doc.extracted_data->>'issuer_name'),'')
    end;
  end if;

  v_counterparty_country:=case
    when v_kind='sales_invoice' then nullif(upper(trim(v_doc.extracted_data->>'customer_country')),'')
  ...
  if coalesce(v_doc.extracted_data->>'notes','') ilike '%transaction statement%'
    ...
    v_counterparty:=split_part(v_description,' · ',1);
