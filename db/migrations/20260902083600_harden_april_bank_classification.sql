-- April QA: make bank-only classification safer for ordinary business owners.
-- Generic ACD payments remain a temporary tax prepayment until a notice establishes the liability.
-- POST monthly account/package fees use the dedicated bank-account-charges account.
...
begin
  ...
  if v_text like '%compte courant associ%' or v_text like '%apport associ%' then
    v_code := '4712'; v_conf := 0.94; v_kind := 'shareholder_transfer';
    v_reason := case when p_direction='income' then 'Shareholder money entered the company. This normally increases the amount the company owes the shareholder (current account).' else 'The company paid the shareholder. This normally reduces the existing shareholder current-account balance; use a shareholder receivable only if the shareholder truly owes the company.' end;
