...
export function InvitePasswordForm({token}:{token:string}){
 ...
 async function submit(event:FormEvent<HTMLFormElement>){
  ...
  try{
   ...
   const{error:inviteError}=await supabase.rpc("accept_organization_invitation",{p_token:token});
 ...
 return <form className={styles.passwordForm} onSubmit={submit}><div className={styles.passwordIntro}><span><LockKeyhole size={16}/></span><div><strong>Create your Compta password</strong><small>Your email is confirmed. Choose the password you will use for future sign-ins.</small></div></div><label><span>Password</span><div className={styles.passwordField}><input type={show?"text":"password"} value={password} onChange={e=>setPassword(e.target.value)} minLength={8} autoComplete="new-password" placeholder="At least 8 characters" required/><button type="button" onClick={()=>setShow(value=>!value)} aria-label={show?"Hide password":"Show password"}>{show?<EyeOff size={16}/>:<Eye size={16}/>}</button></div></label><label><span>Confirm password</span><input type={show?"text":"password"} value={confirm} onChange={e=>setConfirm(e.target.value)} minLength={8} autoComplete="new-password" placeholder="Repeat your password" required/></label>{message?<p className={styles.error}>{message}</p>:null}<button className={styles.accept} type="submit" disabled={busy}>{busy?<LoaderCircle className={styles.spin} size={16}/>:null}{busy?"Activating access…":"Create password & join"}</button></form>;
}
