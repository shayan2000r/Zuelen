...
export default async function OverviewPage(){
 ...
 return <div className={styles.page}>
  ...
  <section className={styles.metricGrid}>
   <Link href="/app/reports" className={styles.metricCard}><span className={styles.metricIcon}><WalletCards size={17}/></span><div><small>Revenue</small><strong>{money(revenue,currency)}</strong></div><ArrowUpRight className={styles.metricArrow} size={16}/></Link>
   <Link href="/app/transactions?status=posted" className={styles.metricCard}><span className={styles.metricIcon}><ReceiptText size={17}/></span><div><small>Expenses</small><strong>{money(expenses,currency)}</strong></div><ArrowUpRight className={styles.metricArrow} size={16}/></Link>
   <Link href="/app/vat" className={styles.metricCard}><span className={styles.metricIcon}><Landmark size={17}/></span><div><small>VAT position</small><strong>{money(Math.abs(vat),currency)}</strong></div><ArrowUpRight className={styles.metricArrow} size={16}/></Link>
   <Link href="/app/taxes" className={styles.metricCard}><span className={styles.metricIcon}><Sparkles size={17}/></span><div><small>Estimated taxes</small><strong>{money(estimatedTaxes,currency)}</strong></div><ArrowUpRight className={styles.metricArrow} size={16}/></Link>
  </section>
  <section className={styles.mainGrid}>
   <article className={styles.overviewCard}><div className={styles.cardHead}><div><p>Overview</p><h2>Revenue & expenses</h2></div><div className={styles.legend}><span><i className={styles.revenueDot}/>Revenue</span><span><i className={styles.expenseDot}/>Expenses</span></div></div><div className={styles.chartSummary}><div><span>Net result</span><strong className={profit>=0?styles.positive:styles.negative}>{money(profit,currency)}</strong></div><div><span>Margin</span><strong>{revenue?`${Math.round((profit/revenue)*100)}%`:"—"}</strong></div></div><div className={styles.barChart}>{monthly.map((month,index)=><div className={styles.barGroup} key={monthKeys[index].key}><div className={styles.bars}><span className={styles.revenueBar} style={{height:`${Math.max(month.revenue?4:0,(month.revenue/chartMax)*100)}%`}}/><span className={styles.expenseBar} style={{height:`${Math.max(month.expense?4:0,(month.expense/chartMax)*100)}%`}}/></div><small>{monthKeys[index].label}</small></div>)}</div></article>
   <article className={styles.aiCard}><div className={styles.aiTop}><div><p>Compta Copilot</p><h2>Your books, explained simply.</h2></div><span><Sparkles size={18}/></span></div><div className={styles.aiIllustration} aria-hidden="true"><div className={styles.aiTileOne}><span>€</span><b>{Math.max(0,Math.round(profit/1000))}k</b></div><div className={styles.aiOrb}><Sparkles size={23}/></div><div className={styles.aiTileTwo}><i/><i/><i/></div><div className={styles.aiGridDots}/></div><p>Ask about VAT, cash, tax reserve, invoices or what is blocking year-end. Answers use the selected financial year.</p><Link href="/app/copilot" className={styles.aiAction}>Open Copilot <ArrowRight size={14}/></Link></article>
  </section>
  <article className={styles.activityCard}><div className={styles.cardHead}><div><p>Recent activity</p><h2>Latest transactions</h2></div><Link href="/app/transactions">View all <ArrowRight size={13}/></Link></div>{recent.length===0?<div className={styles.empty}>No transactions in the selected financial year yet.</div>:<div className={styles.activityList}>{recent.map(row=>{const income=row.direction==="income";return <div className={styles.activityRow} key={row.id}><span className={`${styles.activityIcon} ${income?styles.incomeIcon:""}`}>{income?<ArrowDownLeft size={15}/>:<ArrowUpRight size={15}/>}</span><div><strong>{row.counterparty_name||row.description||(income?"Income":"Expense")}</strong><small>{new Date(`${row.occurred_on}T12:00:00`).toLocaleDateString("en-LU",{day:"2-digit",month:"short"})} · {row.classification_status==="posted"?"Posted":"Needs review"}</small></div><b className={income?styles.incomeAmount:""}>{income?"+":"−"}{money(Number(row.amount_gross),row.currency)}</b></div>})}</div>}</article>
 </div>;
}
