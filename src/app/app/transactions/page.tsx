...
export default async function TransactionsPage({searchParams}:{searchParams:SearchParams}){
 ...
 return <V2Page>
  ...
  <DataSummary items={[
   ...
   {label:fr?"À vérifier":"Needs Review",value:pendingRows.length,icon:CircleHelp,tone:pendingRows.length?"warning":"success"},
   {label:fr?"Comptabilisées":"Posted",value:postedCount,icon:CheckCircle2,tone:"success"},
   {label:fr?"Suggestions":"Suggestions",value:suggestedCount,icon:Sparkles,tone:suggestedCount?"info":"neutral"}
  ]}/>
   ...
   <DataPanel>
    ...
    <DataToolbar>
     <form className={styles.tools} method="get">
      ...
      <select name="status" defaultValue={status}><option value="all">{fr?"Toutes les transactions":"All Transactions"}</option><option value="review">{fr?"À vérifier":"Needs Review"}</option><option value="posted">{fr?"Comptabilisées":"Posted"}</option><option value="suggestions">{fr?"Suggestions":"Suggestions"}</option></select>
