...
export default async function TransactionsPage() {
  ...
  const pendingRows = rows.filter((row) => ["unclassified", "review", "classified"].includes(row.classification_status));
  const nextReview = pendingRows[0] ?? null;

  return (
    <div className={styles.liveWrap}>
      <div className={styles.liveIntro}><div><p className={styles.eyebrow}>Bookkeeping source</p><h1>Transactions</h1><p>Capture business activity first, then deliberately classify it. Posted records stay correctable through automatic reversals instead of silent ledger edits.</p></div><span className={styles.liveBadge}><span>{pendingRows.length}</span> to review · {rows.length} active</span></div>
      <section className={styles.transactionLayout}>
        ...
        <div className={styles.transactionRight}>
          ...
          <article className={styles.ledgerPanel}>
            <div className={styles.ledgerHead}><div><p className={styles.eyebrow}>Activity log</p><h2>Recorded transactions</h2></div><span>Edit or delete anytime</span></div>
            {rows.length === 0 ? <div className={styles.emptyState}><h3>No transactions yet.</h3><p>Use the form to record the first real activity in this workspace.</p></div> : (
              <table className={styles.ledgerTable}>
                <thead><tr><th>Date</th><th>Transaction</th><th>Status</th><th>Amount</th><th aria-label="Actions" /></tr></thead>
                <tbody>{rows.map((row) => { const income=row.direction==="income"; const entryNumber=row.posted_journal_entry_id?entryNumbers.get(row.posted_journal_entry_id):undefined; return <tr key={row.id}>
                  ...
                  <td><TransactionRowActions row={row} /></td>
                </tr>; })}</tbody>
              </table>
            )}
          </article>
