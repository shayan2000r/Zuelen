...
type SearchParams=Promise<{q?:string;status?:string;create?:string}>;
...
export default async function TransactionsPage({searchParams}:{searchParams:SearchParams}){
 ...
 if(error)throw new Error(fr?`Impossible de charger les transactions : ${error.message}`:`Could not load transactions: ${error.message}`);if(accountError)throw new Error(fr?`Impossible de charger les catégories comptables : ${accountError.message}`:`Could not load accounting categories: ${accountError.message}`);if(pcnError)throw new Error(fr?`Impossible de charger le PCN : ${pcnError.message}`:`Could not load the Luxembourg PCN: ${pcnError.message}`);
 ...
 const noResults=rows.length>0&&visibleRows.length===0,reviewComplete=status==="review"&&needsInputRows.length===0;
 ...
 return <V2Page>
  ...
  <DataSummary items={[
   ...
   {label:fr?"Suggestions":"Suggestions",value:suggestedRows.length,icon:Sparkles,tone:suggestedRows.length?"info":"neutral"},
   {label:fr?"À vérifier":"Needs Review",value:needsInputRows.length,icon:CircleHelp,tone:needsInputRows.length?"warning":"success"}
  ]}/>
