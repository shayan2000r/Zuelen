...
export default async function TransactionsPage({searchParams}:{searchParams:SearchParams}){
 ...
 return <div className={styles.page}>
  <div className={styles.intro}><div><p>Bookkeeping source · {year}</p><h1>Transactions</h1><h2>{editable?`Review, classify and manage ${year} activity without losing the audit trail.`:`Read-only access to ${year} transaction activity and posting history.`}</h2></div><span className={styles.badge}><b>{pendingRows.length}</b> to review · {rows.length} active</span></div>
  {editable?<SourceTransactionForm defaultDate={defaultDate}/>:<div className={styles.reviewBanner}><span><Eye size={16}/></span><div><strong>Viewer access · read only</strong><small>You can inspect transactions and journal references, but you cannot create, edit, post or delete activity.</small></div></div>}
  {editable&&pendingRows.length?<div className={styles.bulkWrap}><TransactionBulkActions suggestedCount={suggestedCount} unresolvedCount={unresolvedCount}/></div>:null}
  {editable&&nextReview?<div className={styles.bulkWrap}><TransactionReviewCard transaction={nextReview} accounts={accounts}/></div>:editable&&!nextReview?<div className={styles.reviewBanner}><span><CheckCircle2 size={16}/></span><div><strong>{year} accounting inbox cleared</strong><small>Every active transaction in this financial year has been classified and posted.</small></div></div>:null}
  <section className={styles.panel}><div className={styles.panelHead}><div><p>Activity log · {year}</p><h2>Recorded transactions</h2></div><span>{visibleRows.length} shown · newest first</span></div>
   ...
   {visibleRows.length===0?<div className={styles.empty}>{rows.length?"No transactions match these filters.":editable?`No transactions in financial year ${year} yet. Use the floating button to add your first one.`:`No transactions in financial year ${year}.`}</div>:<TransactionTable rows={visibleRows} accounts={accounts} readOnly={!editable}/>} 
  </section>
 </div>;
}
