...
export default async function TransactionsPage({searchParams}:{searchParams:SearchParams}){
  ...
  const visibleRows=rows.filter(row=>{const statusMatch=status==="all"||(status==="posted"?row.classification_status==="posted":status==="review"?row.classification_status!=="posted":true);if(!statusMatch)return false;if(!q)return true;return [row.counterparty_name,row.description,row.currency,row.classification_status].filter(Boolean).join(" ").toLowerCase().includes(q)});
  return <div className={styles.liveWrap}>
    <div className={styles.liveIntro}><div><p className={styles.eyebrow}>Bookkeeping source</p><h1>Transactions</h1><p>Review, classify and post activity without losing the audit trail.</p></div><span className={styles.liveBadge}><span>{pendingRows.length}</span> to review · {rows.length} active</span></div>
    ...
    <section className={`${styles.transactionLayout} compta-single-column`}><div className={styles.transactionRight}>
