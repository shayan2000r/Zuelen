...
export default async function TransactionsPage({searchParams}:{searchParams:SearchParams}){
 const params=await searchParams,q=(params.q??"").trim().toLowerCase(),status=params.status??"all";const workspace=await getWorkspace();if(!workspace.authenticated)redirect("/sign-in");if(!workspace.company)redirect("/setup");const supabase=await createClient();
 ...
 return <div className={styles.page}>
  <div className={styles.intro}><div><p>Bookkeeping source</p><h1>Transactions</h1><h2>Review, classify and manage activity without losing the audit trail.</h2></div><span className={styles.badge}><b>{pendingRows.length}</b> to review · {rows.length} active</span></div>
  ...
  {nextReview?<div className={styles.bulkWrap}><TransactionReviewCard transaction={nextReview} accounts={accounts}/></div>:<div className={styles.reviewBanner}><span><CheckCircle2 size={16}/></span><div><strong>Accounting inbox cleared</strong><small>Every active transaction has been classified and posted. New activity will appear here for review.</small></div></div>}
  <section className={styles.panel}><div className={styles.panelHead}><div><p>Activity log</p><h2>Recorded transactions</h2></div><span>{visibleRows.length} shown · newest first</span></div>
   <form className={styles.tools} method="get"><label><Search size={15}/><input name="q" defaultValue={params.q??""} placeholder="Search transactions"/></label><select name="status" defaultValue={status}><option value="all">All statuses</option><option value="review">Needs review</option><option value="posted">Posted</option></select><button type="submit">Apply filters</button></form>
   {visibleRows.length===0?<div className={styles.empty}>{rows.length?"No transactions match these filters.":"No transactions yet. Use the floating button to add your first one."}</div>:<TransactionTable rows={visibleRows} accounts={accounts}/>} 
  </section>
 </div>;
}
