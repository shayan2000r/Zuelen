...
export async function aiReanalyzePendingTransactions(_previous:TransactionActionState):Promise<TransactionActionState>{
 const ctx=await context();if(!ctx)return{status:"error",message:"Your session expired. Please sign in again."};if(!process.env.OPENAI_API_KEY)return{status:"error",message:"OpenAI bookkeeping is not configured on this deployment yet."};
