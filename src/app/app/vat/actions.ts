...
export async function prepareVatFilingAction(_previous:VatFilingState,formData:FormData):Promise<VatFilingState>{
 ...
 const s=await createClient();const{data,error}=await s.rpc("prepare_vat_filing",{p_company_id:w.company.id,p_period_start:start,p_period_end:end});if(error)return{status:"error",message:userFacingDataError(error)};
