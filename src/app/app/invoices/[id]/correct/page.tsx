...
export default async function CorrectInvoicePage({params}:{params:Promise<{id:string}>}){
  const{id}=await params;const workspace=await getWorkspace();if(!workspace.authenticated)redirect("/sign-in");if(!workspace.company)redirect("/setup");if(!canBookkeep(workspace.role))redirect(`/app/invoices/${id}`);const compliance=await getInvoiceComplianceStatus(workspace);const supabase=await createClient();
  const{data:invoice,error}=await supabase.from("sales_invoices").select("*").eq("id",id).eq("company_id",workspace.company.id).maybeSingle();if(error)throw new Error(`Could not load invoice: ${error.message}`);if(!invoice||invoice.status!=="issued")notFound();if(invoice.payment_status!=="unpaid")redirect(`/app/invoices/${id}`);
