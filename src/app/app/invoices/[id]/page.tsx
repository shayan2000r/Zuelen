...
function address(snapshot: Snapshot) {
  const raw = snapshot.address ?? snapshot.registered_address;
  return raw && typeof raw === "object" ? raw as Record<string, unknown> : {};
}

function addressValue(raw: Record<string, unknown>, key: string) {
  const item = raw[key];
  return typeof item === "string" ? item : "";
}

function money(amount: number, currency: string) {
  return new Intl.NumberFormat("en-LU", { style: "currency", currency, minimumFractionDigits: 2 }).format(amount);
}
...
export default async function InvoiceDetailPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  const { data: lineData, error: lineError } = await supabase.from("sales_invoice_lines").select("id,line_number,description,quantity,unit_price,vat_rate,net_amount,vat_amount,gross_amount").eq("invoice_id", invoice.id).order("line_number", { ascending: true });
  if (lineError) throw new Error(`Could not load invoice lines: ${lineError.message}`);
  ...
  return (
    <div className={styles.invoiceDetailWrap}>
      <div className={styles.invoiceDetailActions}>
        <Link href="/app/invoices" className={styles.backLink}><ArrowLeft size={14} />Invoices</Link>
        <div><span className={styles.immutableBadge}><LockKeyhole size={13} />Issued document locked</span><InvoicePrintButton /></div>
      </div>

      <article className={`${styles.invoicePaper} ${styles.detailPaper}`}>
        <header className={styles.invoiceHeader}>
          <div><div className={styles.paperBrand}>C</div><strong>{value(issuer, "legal_name")}</strong><small>{value(issuer, "legal_form")}</small></div>
          <div className={styles.invoiceWord}><span>INVOICE</span><small>{invoice.invoice_number}</small></div>
        </header>

        <div className={styles.invoiceMetaGrid}>
          <div><span>FROM</span><strong>{value(issuer, "legal_name")}</strong><p>{addressValue(issuerAddress, "street")}<br />{addressValue(issuerAddress, "postal_code")} {addressValue(issuerAddress, "city")}<br />Luxembourg</p></div>
          <div><span>BILL TO</span><strong>{value(customer, "name")}</strong><p>{addressValue(customerAddress, "street")}<br />{addressValue(customerAddress, "postal_code")} {addressValue(customerAddress, "city")}<br />{value(customer, "country_code")}</p>{value(customer, "vat_number") ? <small>VAT {value(customer, "vat_number")}</small> : null}</div>
        </div>
        ...
        <div className={styles.previewTable}>
          ...
          {lines.map((line) => <div className={styles.previewTableRow} key={line.id}><span>{line.description}<small>{Number(line.vat_rate)}% VAT</small></span><span>{Number(line.quantity)}</span><span>{money(Number(line.unit_price), invoice.currency)}</span><strong>{money(Number(line.net_amount), invoice.currency)}</strong></div>)}
        </div>
