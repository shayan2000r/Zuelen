...
export default async function InvoicesPage(){
 ...
 return <V2Page>
  <PageHeader eyebrow={fr?`Ventes et créances · ${year}`:`Sales & receivables · ${year}`} title={fr?"Factures":"Invoices"} description={editable?(fr?"Créez des brouillons, émettez des factures et suivez les créances de l'exercice sélectionné.":"Create drafts, issue invoices and inspect receivables in the selected financial year."):(fr?"Accès en lecture seule aux factures, brouillons et créances de l'exercice sélectionné.":"Read-only access to invoices, drafts and receivables in the selected financial year.")} actions={editable?[{label:fr?"Créer":"Create",href:"/app/invoices/new",icon:Plus,variant:"primary"}]:[]}/>
  <DataSummary items={[
   {label:fr?"Encours":"Outstanding",value:money(outstanding,currency,locale),icon:ReceiptText,tone:outstanding>0?"info":"neutral"},
   {label:fr?"En retard":"Overdue",value:overdue,icon:Clock3,tone:overdue?"warning":"success"},
   {label:fr?"Brouillons":"Drafts",value:drafts.length,icon:Pencil},
   {label:fr?"Payées":"Paid",value:paid,icon:CircleCheck,tone:"success"}
  ]}/>
