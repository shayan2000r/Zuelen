...
import { ArrowLeft, BadgeCheck, BriefcaseBusiness, CheckCircle2, Languages, MapPin, MonitorSmartphone, Sparkles, UsersRound } from "lucide-react";
import { notFound, redirect } from "next/navigation";
import { AccountantEngagement } from "@/components/accountant-engagement";
import { accountantInitials, type AccountantListingSubscription, type AccountantProfile } from "@/lib/accountants";
...
export default async function AccountantProfilePage({ params }: { params: Promise<{ slug: string }> }) {
  ...
  return <div className={styles.page}>
    ...
    <section className={`${styles.profileCard} ${premium ? styles.premium : ""}`}>
      {premium ? <div className={styles.featured}><Sparkles size={13}/>Featured professional</div> : null}
      <div className={styles.identity}>
        ...
        <div><div className={styles.nameLine}><h1>{profile.full_name}</h1><BadgeCheck size={19}/></div><p>{profile.professional_title}</p>{profile.firm_name ? <strong>{profile.firm_name}</strong> : null}<div className={styles.meta}>{profile.location ? <span><MapPin size={13}/>{profile.location}</span> : null}{profile.languages.length ? <span><Languages size={13}/>{profile.languages.join(" · ")}</span> : null}</div></div>
      </div>
      <div className={styles.contactPanel}><span>{fr ? "Contacter ce professionnel" : "Contact this professional"}</span><h2>{profile.accepting_new_clients ? (fr ? "Accepte de nouveaux clients" : "Accepting new clients") : (fr ? "Disponibilité limitée" : "Limited availability")}</h2><AccountantEngagement profileId={profile.id} email={profile.email} phone={profile.phone} website={profile.website}/></div>
    </section>

    <section className={styles.contentGrid}>
      <article className={styles.main}><div className={styles.block}><span className={styles.label}>{fr ? "À propos" : "About"}</span><h2>{fr ? "À propos de ce professionnel" : "About this professional"}</h2><p>{profile.bio || (fr ? "Aucune biographie n’a encore été ajoutée." : "No biography has been added yet.")}</p></div>
      <div className={styles.block}><span className={styles.label}>{fr ? "Expertise" : "Expertise"}</span><h2>{fr ? "Spécialités" : "Specialties"}</h2><div className={styles.tags}>{profile.specialties.map(item => <span key={item}>{item}</span>)}</div></div>
      <div className={styles.block}><span className={styles.label}>{fr ? "Clientèle" : "Clients"}</span><h2>{fr ? "Types d’entreprises accompagnées" : "Businesses they work with"}</h2><div className={styles.tags}>{profile.business_types.length ? profile.business_types.map(item => <span key={item}>{item}</span>) : <span>{fr ? "PME et indépendants" : "SMEs and independent businesses"}</span>}</div></div></article>
