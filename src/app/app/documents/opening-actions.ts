...
export async function processOpeningDocumentAction(_previous:OpeningImportState,formData:FormData):Promise<OpeningImportState>{
 ...
 try{
  ...
  const codes=openingLines.map(line=>line.code),{data:master,error:masterError}=await supabase.from("pcn_accounts").select("id,code,label_en,label_fr,account_type").eq("source_version","PCN2020").eq("is_active",true).in("code",codes);if(masterError)throw new Error(masterError.message);
