...
export async function processOpeningDocumentAction(_previous:OpeningImportState,formData:FormData):Promise<OpeningImportState>{
 ...
 try{
  ...
  const companyMap=new Map((companyAccounts??[]).map(account=>[account.code,account])),posting=clean.map(line=>({account_id:companyMap.get(line.code)?.id,debit:line.debit,credit:line.credit}));if(posting.some(line=>!line.account_id))throw new Error("Zuelen could not map every extracted PCN account to the company ledger.");
  ...
  if(totalDebit<=0||Math.abs(totalDebit-totalCredit)>.005){await supabase.from("documents").update({extraction_status:"needs_review",extracted_data:{...extracted,opening_import_status:"unbalanced",target_fiscal_year:targetYear,total_debit:totalDebit,total_credit:totalCredit,leaf_codes:clean.map(line=>line.code),model:"gpt-5-mini",extracted_at:new Date().toISOString()}}).eq("id",doc.id);refresh();return{status:"error",sourceYear,targetYear,lineCount:clean.length,message:`Zuelen extracted ${clean.length} balance-sheet accounts, but the position does not balance (${totalDebit.toFixed(2)} debit / ${totalCredit.toFixed(2)} credit). Nothing was posted.`}}
