...
const REPORTS={
 ...
 pcn:"PCN Account Balances",
 annual_accounts:"Annual Accounts",
 annexe:"Annexe to the Annual Accounts",
 general_ledger:"General Ledger"
} as const;
type ReportType=keyof typeof REPORTS;
const GENERATOR_VERSION="2026.1";
...
export async function generateFinancialDocumentAction(_previous:GeneratedDocumentState,formData:FormData):Promise<GeneratedDocumentState>{
 ...
 const[{data:period,error:periodError},{data:filing,error:filingError}]=await Promise.all([
  supabase.from("accounting_periods").select("status,locked_at").eq("company_id",workspace.company.id).eq("starts_on",`${fiscalYear}-01-01`).eq("ends_on",`${fiscalYear}-12-31`).maybeSingle(),
  supabase.from("filings").select("id,period_label,status,rules_version,ledger_snapshot,snapshot_at,ledger_checksum,period_start,period_end").eq("company_id",workspace.company.id).eq("filing_type","ecdf_accounts").eq("period_label",String(fiscalYear)).order("snapshot_at",{ascending:false}).limit(1).maybeSingle()
 ]);
 if(periodError)return{status:"error",message:periodError.message};if(filingError)return{status:"error",message:filingError.message};
 ...
 const payload:Record<string,unknown>={
  ...
  snapshot_at:filing.snapshot_at,
  ledger_checksum:filing.ledger_checksum,
  rules_version:filing.rules_version,
 ...
 if(documentType==="general_ledger"){
  ...
  if(entryIds.length){
   const[entriesResult,linesResult,accountsResult]=await Promise.all([
    supabase.from("journal_entries").select("id,entry_number,entry_date,description,source_type").in("id",entryIds).order("entry_number",{ascending:true}),
    supabase.from("journal_lines").select("journal_entry_id,company_account_id,description,debit,credit,currency").in("journal_entry_id",entryIds).order("created_at",{ascending:true}),
    supabase.from("company_accounts").select("id,code,label,account_type").eq("company_id",workspace.company.id)
   ]);
   if(entriesResult.error||linesResult.error||accountsResult.error)return{status:"error",message:"Compta could not freeze the general-ledger detail."};
   const accountMap=new Map((accountsResult.data??[]).map(a=>[a.id,a])),linesByEntry=new Map<string,Record<string,unknown>[]>();
   for(const line of linesResult.data??[]){const account=accountMap.get(line.company_account_id),rows=linesByEntry.get(line.journal_entry_id)??[];rows.push({account_code:account?.code??"",account_label:account?.label??line.description??"Account",account_type:account?.account_type??"",description:line.description,debit:Number(line.debit),credit:Number(line.credit),currency:line.currency});linesByEntry.set(line.journal_entry_id,rows)}
   payload.general_ledger={entries:(entriesResult.data??[]).map(entry=>({id:entry.id,entry_number:entry.entry_number,entry_date:entry.entry_date,description:entry.description,source_type:entry.source_type,lines:linesByEntry.get(entry.id)??[]}))};
  }else payload.general_ledger={entries:[]};
 }
