...
export default async function YearEndPage(){const w=await getWorkspace();if(!w.authenticated)redirect("/sign-in");if(!w.company)redirect("/setup");const s=await createClient(),year=new Date().getFullYear(),currency=w.company.base_currency||"EUR",from=`${year}-01-01`,to=`${year}-12-31`;
 const [pending,bank,drafts,unpaid,docs,vat,entries,shareAccounts,snapshot,period]=await Promise.all([
 ...
 ];const ready=checks.every(c=>c.ok),hasSnapshot=Boolean(snapshot.data),hardClosed=period.data?.status==="hard_closed";
 return <div className={styles.page}><div className={styles.intro}><div><p>Year-end · {year}</p><h1>{hardClosed?"The year is locked.":hasSnapshot?"Closing snapshot frozen.":ready?"The books are structurally ready.":"Close the gaps before you close the year."}</h1><span>{hardClosed?"This period no longer accepts operational postings. Corrections require a controlled accounting process outside the closed period.":hasSnapshot?"Annual-account preparation is anchored to a frozen ledger snapshot.":"Compta checks evidence, reconciliation and ledger integrity before allowing a close."}</span></div><div className={`${styles.ready} ${(ready||hasSnapshot)?styles.readyGood:""}`}>{(ready||hasSnapshot)?<CheckCircle2/>:<CircleDashed/>}{hardClosed?"Hard closed":hasSnapshot?"Snapshot frozen":ready?"Ready for closing review":"Closing blockers remain"}</div></div>
