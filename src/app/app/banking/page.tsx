...
export default async function BankingPage({searchParams}:{searchParams:SearchParams}){
 ...
 if(accountsResult.error)throw new Error(`Could not load bank accounts: ${accountsResult.error.message}`);if(bankResult.error)throw new Error(`Could not load bank transactions: ${bankResult.error.message}`);if(batchesResult.error)throw new Error(`Could not load bank imports: ${batchesResult.error.message}`);
