...
export async function importBankRows(_previous:BankImportState,formData:FormData):Promise<BankImportState>{
 ...
 try{await assertUsageAvailable(workspace.organization.id,"bank_imports",1);await assertUsageAvailable(workspace.organization.id,"transactions",rows.length)}catch(error){return{status:"error",message:billingLimitMessage(error,locale)??(error instanceof Error?error.message:"Your Basic allowance has been reached.")}}
 const{data,error}=await supabase.rpc("import_bank_rows",{p_company_id:workspace.company.id,p_account_name:effectiveName,p_iban:effectiveIban||null,p_currency:currency,p_file_name:fileName||null,p_rows:rows});if(error){const friendly=billingLimitMessage(new Error(error.message),locale);return{status:"error",message:friendly??error.message}}const result=data&&typeof data==="object"?data as Record<string,unknown>:{},imported=Number(result.imported??0),duplicates=Number(result.duplicates??0),batchId=typeof result.batch_id==="string"?result.batch_id:null;
