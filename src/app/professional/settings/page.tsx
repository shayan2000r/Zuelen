...
export default async function ProfessionalSettingsPage({ searchParams }: { searchParams: Promise<{ saved?: string; error?: string }> }) {
  ...
  >
    <div className={styles.page}>
      ...
      <form action={saveProfessionalSettingsAction} className={`${styles.card} ${styles.settingsForm}`}>
        <div className={styles.settingsSection}>
          <div className={styles.settingsIcon}><Globe2 size={18}/></div>
          <div className={styles.settingsCopy}><h2>{l("Language", "Langue")}</h2><p>{l("Choose the language used across your professional workspace and service emails.", "Choisissez la langue utilisée dans votre espace professionnel et dans les e-mails de service.")}</p></div>
          <select name="locale" defaultValue={locale} className={styles.settingsSelect} aria-label={l("Language", "Langue")}><option value="en">English</option><option value="fr">Français</option></select>
        </div>

        <div className={styles.settingsDivider}/>

        <div className={styles.settingsSection}>
          <div className={styles.settingsIcon}><Bell size={18}/></div>
          <div className={styles.settingsCopy}><h2>{l("Email notifications", "Notifications par e-mail")}</h2><p>{l("Receive optional directory and Zuelen professional product updates.", "Recevez les actualités facultatives de l’annuaire et du produit Zuelen Professionals.")}</p><small><ShieldCheck size={13}/>{l("Review decisions, billing and security emails are essential service messages and are always sent.", "Les décisions de vérification, la facturation et les e-mails de sécurité sont des messages de service essentiels et sont toujours envoyés.")}</small></div>
          <label className={styles.settingsToggle}><input type="checkbox" name="professional_email_updates" defaultChecked={optionalUpdates}/><span aria-hidden="true"/><em>{optionalUpdates ? l("On", "Activé") : l("Off", "Désactivé")}</em></label>
        </div>

        <div className={styles.settingsFooter}><button className={styles.primary} type="submit">{l("Save settings", "Enregistrer")}</button></div>
      </form>
      ...
      <section className={styles.card}>
        ...
        <div className={styles.workspaceSetting}>
      ...
      <section className={`${styles.card} ${styles.dangerCard}`}>
        ...
        {deletionBlocked ? <div className={styles.dangerBlocked}><AlertTriangle size={16}/><div><strong>{l("Subscription must be closed first", "L’abonnement doit d’abord être clôturé")}</strong><span>{l("Open Subscription & billing to cancel the directory subscription. Deletion is available once the billable subscription has ended.", "Ouvrez Abonnement & facturation pour annuler l’abonnement à l’annuaire. La suppression sera disponible une fois l’abonnement facturable terminé.")}</span></div><Link href="/professional/billing" className={styles.secondaryLink}>{l("Open billing", "Ouvrir la facturation")}</Link></div> : <form action={deleteProfessionalProfileAction} className={styles.deleteForm}><label><span>{l("Type DELETE to confirm", "Saisissez DELETE pour confirmer")}</span><input name="confirmation" autoComplete="off" placeholder="DELETE" required/></label><button type="submit" className={styles.dangerButton}><Trash2 size={14}/>{l("Delete professional profile", "Supprimer le profil professionnel")}</button></form>}
      </section>
    </div>
  </ProfessionalFrame>;
}
