...
export default async function SetupPage({ searchParams }: { searchParams: Promise<{ add?: string }> }) {
  ...
  return <main className={styles.shell}>
    ...
    <section className={styles.intro}><span><Plus size={14}/>{additive ? l("Add another activity", "Ajouter une autre activité") : l("Workspace setup", "Configuration de l’espace")}</span><h1>{additive ? l("What would you like to add?", "Que souhaitez-vous ajouter ?") : l("What would you like to set up?", "Que souhaitez-vous configurer ?")}</h1><p>{l("Choose what best describes how you'll use Zuelen. You can add another activity later.", "Choisissez ce qui décrit le mieux votre utilisation de Zuelen. Vous pourrez ajouter une autre activité plus tard.")}</p></section>
    <section className={styles.grid} aria-label={l("Setup options", "Options de configuration")}>{cards.map(card => <article className={styles.card} key={card.href}><div className={styles.icon}><card.icon size={23}/></div><h2>{card.title}</h2><p>{card.description}</p><strong className={styles.examples}>{card.examples}</strong><small>{card.detail}</small><Link href={card.href}>{card.cta}<span aria-hidden="true">→</span></Link></article>)}</section>
    <p className={styles.footer}>{l("Your choice is not permanent. One secure Zuelen login can hold several activities and a professional profile.", "Votre choix n’est pas définitif. Un même compte Zuelen sécurisé peut contenir plusieurs activités et un profil professionnel.")}</p>
  </main>;
}
