...
import { ACCOUNTANT_BUSINESS_TYPES, ACCOUNTANT_LANGUAGES, ACCOUNTANT_SPECIALTIES, accountantInitials, accountantLanguageLabel, accountantStripeConfigured, type AccountantListingSubscription, type AccountantProfile } from "@/lib/accountants";
...
export default async function AccountantManagePage({ searchParams }: { searchParams: Promise<{ saved?: string; checkout?: string; plan?: string }> }) {
  ...
  if (!profile) redirect("/professional");
  ...
  return <main className={styles.shell}>
    <header className={styles.topbar}>
      <Link href={workspace.company ? "/app/accountants" : "/accountants/directory"} className={styles.brand}><img src="/zuelen-icon.png" alt="" /><strong>Zuelen</strong><span>Professionals</span></Link>
      <Link href={workspace.company ? "/app/accountants" : "/accountants/directory"} className={styles.back}><ArrowLeft size={14}/>Back to directory</Link>
    </header>

    <div className={styles.page}>
      <section className={styles.hero}>
        <div><span className={styles.eyebrow}><BriefcaseBusiness size={14}/> Zuelen for accountants</span><h1>Your practice, in front of businesses that need you.</h1><p>Manage your trusted professional profile, listing subscription and performance from one dedicated workspace.</p></div>
        <div className={styles.statusCard}><span>Listing status</span><strong className={styles[profile.approval_status]}>{profile.approval_status === "approved" ? <BadgeCheck size={16}/> : <Clock3 size={16}/>} {profile.approval_status}</strong><small>{live ? "Visible in the directory" : profile.approval_status === "pending" ? "Waiting for Zuelen review" : profile.approval_status === "rejected" ? profile.rejection_reason || "Changes are required" : "Subscription required"}</small></div>
      </section>
      ...
      <section className={styles.grid}>
        <form action={saveAccountantProfileAction} encType="multipart/form-data" className={styles.formCard}>
          <div className={styles.sectionHead}><div><span>Professional profile</span><h2>Edit your listing</h2></div>{profile.photo_url ? <img src={profile.photo_url} alt="" className={styles.avatar}/> : <span className={styles.avatarFallback}>{accountantInitials(profile.full_name || workspace.profile?.full_name || workspace.email || "A")}</span>}</div>
          <div className={styles.twoCols}>
            <label><span>Full name *</span><input name="full_name" required defaultValue={profile.full_name} placeholder="Marie Dupont"/></label>
            <label><span>Firm / practice</span><input name="firm_name" defaultValue={profile.firm_name ?? ""} placeholder="Dupont Fiduciaire"/></label>
            <label><span>Professional title *</span><input name="professional_title" required defaultValue={profile.professional_title} placeholder="Accountant / Expert-comptable"/></label>
