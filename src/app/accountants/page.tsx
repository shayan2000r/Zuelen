...
export default async function AccountantsLandingPage() {
  ...
  return <main className={styles.page}>
    <header><Link href="/" className={styles.brand}><img src="/zuelen-icon.png" alt=""/><strong>Zuelen</strong><span>for Accountants</span></Link><Link href={workspace.authenticated ? "/accountants/manage" : "/sign-in?next=/accountants/manage"}>{workspace.authenticated ? "Manage my listing" : "Sign in"}</Link></header>
    <section className={styles.hero}><div><span><BriefcaseBusiness size={14}/>Zuelen professional network</span><h1>Put your practice where Luxembourg businesses already manage their finances.</h1><p>Create a trusted directory profile and receive direct enquiries from business owners looking for accounting expertise.</p><div className={styles.actions}><Link href={workspace.authenticated ? "/accountants/manage" : "/sign-in?next=/accountants/manage"}>Start your 30-day trial <ArrowRight size={15}/></Link><small>Payment method required · cancel before the trial ends</small></div></div><aside><BadgeCheck size={28}/><strong>Curated, not crowded.</strong><p>Every accountant profile is reviewed before it appears in the directory, helping businesses browse with more confidence.</p></aside></section>
    ...
    <section className={styles.trust}><div><ShieldCheck size={20}/><span><strong>30 days free on both plans</strong><small>Try your selected listing before your first monthly charge.</small></span></div><Link href={workspace.authenticated ? "/accountants/manage" : "/sign-in?next=/accountants/manage"}>Create my listing <ArrowRight size={14}/></Link></section>
  </main>;
}
