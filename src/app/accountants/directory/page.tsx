...
export default async function StandaloneAccountantsPage({ searchParams }: { searchParams: Promise<Params> }) {
  ...
  if (workspace.userId) {
    ...
    if (ownProfile) {
      const ownSubscriptionResult = await supabase.from("accountant_listing_subscriptions").select("*").eq("profile_id", ownProfile.id).maybeSingle();
      if (ownSubscriptionResult.error) throw new Error(ownSubscriptionResult.error.message);
      ownSubscription = ownSubscriptionResult.data as AccountantListingSubscription | null;
    }
  }
