...
export default async function StandaloneAccountantsPage({ searchParams }: { searchParams: Promise<Params> }) {
  ...
  return <main style={{minHeight:"100vh",background:"#f7f7f4"}}>
    <header style={{height:72,maxWidth:1280,margin:"0 auto",padding:"0 36px",display:"flex",alignItems:"center",justifyContent:"space-between",borderBottom:"1px solid #e1e4dd"}}><Link href="/" style={{display:"flex",alignItems:"center",gap:9,textDecoration:"none",color:"#202622"}}><img src="/zuelen-icon.png" alt="" style={{width:29,height:29,objectFit:"contain"}}/><strong style={{fontSize:14,letterSpacing:"-.04em"}}>Zuelen</strong><span style={{fontSize:9,color:"#7a8179",borderLeft:"1px solid #d9ddd5",paddingLeft:9}}>Accountant directory</span></Link><div style={{display:"flex",alignItems:"center",gap:14}}>{workspace.company?<Link href="/app" style={{fontSize:10,color:"#68716b",textDecoration:"none"}}>{fr?"Espace entreprise":"Business workspace"}</Link>:null}<Link href="/accountants/manage" style={{fontSize:10,fontWeight:800,color:"#214b30",textDecoration:"none"}}>{fr?"Mon profil professionnel":"My professional profile"}</Link></div></header>
    <div className={styles.page}>
      <section className={styles.hero}>
        <div><span className={styles.eyebrow}><BriefcaseBusiness size={14}/>{fr ? "Réseau professionnel Zuelen" : "Zuelen professional network"}</span><h1>{fr ? "Trouvez un comptable qui comprend votre entreprise." : "Find an accountant who understands your business."}</h1><p>{fr ? "Recherchez des professionnels indépendants selon leur expertise, leurs langues et les entreprises qu’ils accompagnent — sans devoir créer un espace entreprise." : "Search independent professionals by expertise, languages and the businesses they support — without needing to create a company workspace."}</p></div>
