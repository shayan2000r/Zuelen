...
export default async function StandaloneAccountantProfilePage({ params }: { params: Promise<{ slug: string }> }) {
  ...
  return <main style={{minHeight:"100vh",background:"#f7f7f4",paddingTop:24}}><div className={styles.page}>
    ...
    <section className={styles.contentGrid}>
      <article className={styles.main}>
        ...
        <div className={styles.block}><span className={styles.label}>{fr ? "Expertise" : "Expertise"}</span><h2>{fr ? "Spécialités" : "Specialties"}</h2><div className={styles.tags}>{profile.specialties.map(item => <span key={item}>{accountantSpecialtyLabel(item,locale)}</span>)}</div></div>
