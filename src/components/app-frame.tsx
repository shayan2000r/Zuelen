...
function AppFrameInner({ children, companyName, fiscalYear, fiscalYears, email, userName, userRole, userAvatarUrl, attentionCount = 0, brandImageUrl = null, locale, plan, entityKind, capabilities, workspaces, activeWorkspaceId }: AppFrameProps) {
  ...
  return <RoleProvider role={userRole}>
    <main className="app-shell" data-role={userRole ?? "member"} data-locale={locale} data-plan={plan} data-entity-kind={entityKind}>
      <aside className={`sidebar ${mobileNav ? "sidebar-open" : ""}`}>
        ...
        <div className={`${frame.workspaceSwitcherWrap}`}> <button type="button" className={`company-switcher ${frame.workspaceSwitcherButton} ${frame.yearSwitcher}`} onClick={()=>setWorkspaceOpen(value=>!value)} aria-expanded={workspaceOpen}>{companyAvatar("company-avatar")}<span className={`company-copy ${frame.yearCopy}`}><strong>{companyName}</strong><span className={frame.yearSelectWrap}><span className={frame.yearHint}>{independent?l("Independent","Indépendant"):l("Company","Société")} · {tx("financialYear")}</span><span className={frame.yearStatic}>{fiscalYear}</span><span className={frame.companyPlanBadge}>{plan === "premium" ? "Premium" : "Basic"}</span></span></span><ChevronDown size={14} className={workspaceOpen?frame.chevronOpen:""}/></button>{workspaceOpen?<div className={frame.workspaceMenu}><span>{l("Workspaces","Espaces")}</span>{workspaces.map(item=><form action={switchWorkspaceAction} key={item.companyId}><input type="hidden" name="company_id" value={item.companyId}/><input type="hidden" name="return_to" value="/app"/><button type="submit" className={item.companyId===activeWorkspaceId?frame.activeWorkspace:""}><span className={frame.workspaceIcon}>{item.entityKind==="independent"?<UserRound size={15}/>:<Building2 size={15}/>}</span><div><strong>{item.displayName}</strong><small>{item.entityKind==="independent"?l("Independent","Indépendant"):`${l("Company","Société")} · ${item.legalForm}`}</small></div>{item.companyId===activeWorkspaceId?<Check size={14}/>:null}</button></form>)}<Link href="/setup?add=1" onClick={()=>setWorkspaceOpen(false)} className={frame.addWorkspace}><Plus size={15}/>{l("Add another activity","Ajouter une activité")}</Link><div className={frame.workspaceYear}><span>{tx("financialYear")}</span><select value={fiscalYear} disabled={switchingYear} onChange={event=>changeFiscalYear(Number(event.target.value))}>{fiscalYears.map(year=><option key={year} value={year}>{year}</option>)}</select></div></div>:null}</div>
