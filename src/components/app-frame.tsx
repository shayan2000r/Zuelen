...
const collapsibleGroups: CollapsibleGroup[] = [
  { key:"accounting",label:"Accounting & tax",icon:BookOpen,items:[
  ...
  { key:"insights",label:"Insights & AI",icon:BarChart3,items:[
...
export function AppFrame({children,companyName,fiscalYear,fiscalYears,email,userName,userRole,userAvatarUrl,attentionCount=0,brandImageUrl=null}:{children:ReactNode;companyName:string;fiscalYear:number;fiscalYears:number[];email:string|null;userName:string|null;userRole:OrganizationRole|null;userAvatarUrl:string|null;attentionCount?:number;brandImageUrl?:string|null}){
 const pathname=usePathname(),[mobileNav,setMobileNav]=useState(false),[searchOpen,setSearchOpen]=useState(false),[notificationsOpen,setNotificationsOpen]=useState(false),[query,setQuery]=useState(""),[theme,setTheme]=useState<Theme>("light"),[switchingYear,setSwitchingYear]=useState(false);
 ...
 const userLabel=useMemo(()=>userName?.trim()||email?.split("@")[0]||"Member",[userName,email]),roleLabel=prettyRole(userRole);
 const results=useMemo(()=>{const normalized=query.trim().toLowerCase();return normalized?allItems.filter(item=>`${item.label} ${item.description}`.toLowerCase().includes(normalized)):allItems.slice(0,8)},[query]);
 useEffect(()=>{const stored=window.localStorage.getItem("compta-theme"),initialTheme:Theme=stored==="dark"?"dark":"light";setTheme(initialTheme);document.documentElement.dataset.comptaTheme=initialTheme},[]);
 function applyTheme(next:Theme){setTheme(next);window.localStorage.setItem("compta-theme",next);document.documentElement.dataset.comptaTheme=next}
 function changeFiscalYear(next:number){if(!Number.isInteger(next)||next===fiscalYear)return;setSwitchingYear(true);document.cookie=`compta-fiscal-year=${next}; Path=/; Max-Age=31536000; SameSite=Lax`;window.location.reload()}
 useEffect(()=>{for(const group of collapsibleGroups)if(group.items.some(item=>pathname.startsWith(item.href)))setOpenGroups(current=>({...current,[group.key]:true}))},[pathname]);
 ...
 const navLink=(item:NavItem,nested=false)=>{const active=item.href==="/app"?pathname==="/app":pathname.startsWith(item.href);return <Link key={item.href} href={item.href} className={`nav-item ${active?"nav-active":""} ${nested?frame.nestedNavItem:""}`} onClick={()=>setMobileNav(false)}><item.icon size={nested?14:16}/><span>{item.label}</span>{item.label==="Transactions"&&attentionCount>0?<em>{attentionCount}</em>:null}</Link>};
 ...
 return <RoleProvider role={userRole}><main className="app-shell" data-role={userRole??"member"}><aside className={`sidebar ${mobileNav?"sidebar-open":""}`}>
  <div className="brand-row"><Link href="/app" className={styles.brandLink}><div className="brand-mark"><span>C</span></div><div className="brand-word">Compta</div><span className={frame.brandBadge}>LU</span></Link><button className="icon-btn mobile-only" onClick={()=>setMobileNav(false)} aria-label="Close navigation"><X size={18}/></button></div>
  ...
  <nav className="nav-list" aria-label="Primary navigation"><div className={frame.primaryNav}>{primaryItems.map(item=>navLink(item))}</div><div className={frame.groupDivider}/>{collapsibleGroups.map(group=>{const isOpen=openGroups[group.key],hasActiveChild=group.items.some(item=>pathname.startsWith(item.href));return <div className={frame.navGroup} key={group.key}><button type="button" className={`${frame.groupButton} ${hasActiveChild?frame.groupButtonActive:""}`} onClick={()=>setOpenGroups(current=>({...current,[group.key]:!current[group.key]}))} aria-expanded={isOpen}><group.icon size={16}/><span>{group.label}</span><ChevronDown className={isOpen?frame.chevronOpen:""} size={14}/></button><div className={`${frame.groupChildren} ${isOpen?frame.groupChildrenOpen:""}`}><div>{group.items.map(item=>navLink(item,true))}</div></div></div>})}<div className={frame.groupDivider}/>{navLink(settingsItem)}</nav>
  <div className="sidebar-spacer"/><div className={frame.themeRow}><span>Appearance</span><div className={frame.themeToggle} role="group" aria-label="Appearance"><button type="button" className={theme==="light"?frame.themeActive:""} onClick={()=>applyTheme("light")} aria-label="Use light mode"><Sun size={14}/></button><button type="button" className={theme==="dark"?frame.themeActive:""} onClick={()=>applyTheme("dark")} aria-label="Use dark mode"><Moon size={14}/></button></div></div>
  <div className="user-row"><Link href="/app/settings/profile" className={frame.userProfileLink}>{personalAvatar("user-avatar")}<span><strong>{userLabel}</strong><small>{roleLabel} · My Profile</small></span></Link><form action="/auth/signout" method="post" className={styles.signOutForm}><button type="submit" className={styles.signOutButton} aria-label="Sign out" title="Sign out"><LogOut size={15}/></button></form></div>
 </aside>{mobileNav?<button className="scrim" aria-label="Close navigation" onClick={()=>setMobileNav(false)}/>:null}
 <section className="workspace"><header className="topbar"><div className="topbar-left"><button className="icon-btn mobile-only" onClick={()=>setMobileNav(true)} aria-label="Open navigation"><Menu size={18}/></button></div><div className="topbar-actions"><button className="search-btn" type="button" onClick={()=>setSearchOpen(true)} aria-label="Search Compta" title="Search Compta (⌘K)"><Search size={17}/></button><div className={frame.notificationWrap}><button className="icon-btn" type="button" aria-label="Notifications" aria-expanded={notificationsOpen} onClick={()=>setNotificationsOpen(value=>!value)}><Bell size={17}/>{attentionCount>0?<span className="notification-dot"/>:null}</button>{notificationsOpen?<div className={frame.notificationPanel}><div><strong>Notifications</strong><button onClick={()=>setNotificationsOpen(false)} aria-label="Close notifications"><X size={14}/></button></div>{attentionCount?<Link href="/app/transactions" onClick={()=>setNotificationsOpen(false)}><span className={frame.noticeIcon}><WalletCards size={15}/></span><span><strong>{attentionCount} transaction{attentionCount===1?"":"s"} need review</strong><small>Open transaction review for {fiscalYear}</small></span></Link>:<p>You’re all caught up for {fiscalYear}.</p>}<Link href="/app/compliance" onClick={()=>setNotificationsOpen(false)}><span className={frame.noticeIcon}><CalendarCheck2 size={15}/></span><span><strong>Compliance center</strong><small>Review {fiscalYear} obligations</small></span></Link></div>:null}</div><Link className={`icon-btn ${frame.profileButton}`} href="/app/settings/profile" aria-label={`Profile: ${userLabel}`} title="My Profile">{userAvatarUrl?<span className={frame.topAvatar}><img src={userAvatarUrl} alt=""/></span>:<UserRound size={17}/>}</Link></div></header>{children}</section>
