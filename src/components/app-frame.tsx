    ...
    <main className="app-shell">
      <aside className={`sidebar ${mobileNav ? "sidebar-open" : ""}`}>
        ...
        <button className="company-switcher" type="button">
          ...
          <span className="company-copy"><strong>{companyName}</strong><small>{fiscalYear} financial year</small></span>
          <ChevronDown size={16} />
        </button>
        ...
        <nav className="nav-list" aria-label="Primary navigation">
          {nav.map((item) => {
            ...
            if (!item.href) {
              return (
                <span key={item.label} className={`nav-item ${styles.disabledNav}`} aria-disabled="true">
            ...
            return (
              <Link key={item.label} href={item.href} className={`nav-item ${active ? "nav-active" : ""}`} onClick={() => setMobileNav(false)}>
                <item.icon size={18} strokeWidth={1.8} />
                <span>{item.label}</span>
        ...
        <div className="sidebar-insight">
          ...
          <div><strong>Live workspace</strong><p>Protected by company-level RLS.</p></div>
          <span className="health-dot" />
        </div>
        <div className="nav-list nav-list-bottom">
          <span className={`nav-item ${styles.disabledNav}`}><CircleHelp size={18} /><span>Help center</span></span>
          <span className={`nav-item ${styles.disabledNav}`}><Settings size={18} /><span>Settings</span></span>
        </div>
      ...
      <section className="workspace">
        <header className="topbar">
          ...
          <div className="topbar-actions">
            ...
            <button className="icon-btn" type="button" aria-label="Notifications"><Bell size={18} />{attentionCount > 0 ? <span className="notification-dot" /> : null}</button>
          </div>
        </header>
        {children}
      </section>
