...
function AppFrameInner({
  ...
  const overview = useMemo<NavItem>(() => ({
    label: tx("overview"),
    description: tx("companySnapshot"),
  ...
  const results = useMemo(() => {
    ...
    return normalized ? allItems.filter(item => `${item.label} ${item.description}`.toLowerCase().includes(normalized)) : allItems.slice(0, 9);
  }, [query, allItems]);
  ...
  const renderGroup = (group: CollapsibleGroup) => {
    ...
    const hasActiveChild = group.items.some(item => pathname === item.href || pathname.startsWith(`${item.href}/`) || (item.href === "/app/year-end" && pathname.startsWith("/app/ecdf")));
    return (
      <div className={frame.navGroup} key={group.key}>
        <button type="button" className={`${frame.groupButton} ${hasActiveChild ? frame.groupButtonActive : ""}`} onClick={() => setOpenGroups(current => ({ ...current, [group.key]: !current[group.key] }))} aria-expanded={isOpen}>
          <group.icon size={16} />
  ...
  const companyAvatar = (className: string) => brandImageUrl ? (
    <span className={`${className} ${frame.imageAvatar}`}><img src={brandImageUrl} alt="" /></span>
  ) : <span className={className}>{initials(companyName).slice(0, 1) || "C"}</span>;

  const personalAvatar = (className: string) => userAvatarUrl ? (
    <span className={`${className} ${frame.imageAvatar}`}><img src={userAvatarUrl} alt="" /></span>
  ) : <span className={className}>{initials(userLabel) || <UserRound size={16} />}</span>;
      ...
      <main className="app-shell" data-role={userRole ?? "member"} data-locale={locale} data-plan={plan}>
        <aside className={`sidebar ${mobileNav ? "sidebar-open" : ""}`}>
          <div className="brand-row">
            ...
            <button className="icon-btn mobile-only" onClick={() => setMobileNav(false)} aria-label={tx("closeNavigation")}><X size={18} /></button>
          </div>
          ...
          <div className={`company-switcher ${frame.yearSwitcher}`}>
            ...
            <span className={`company-copy ${frame.yearCopy}`}>
              ...
              <span className={frame.yearSelectWrap}>
                <span className={frame.yearHint}>{tx("financialYear")}</span>
                <select className={frame.yearSelect} value={fiscalYear} disabled={switchingYear} onChange={event => changeFiscalYear(Number(event.target.value))} aria-label={tx("financialYear")}>
          ...
          <div className={frame.themeRow}>
            ...
            <div className={frame.themeToggle} role="group" aria-label={tx("appearance")}>
              <button type="button" className={theme === "light" ? frame.themeActive : ""} onClick={() => applyTheme("light")} aria-label={tx("lightMode")}><Sun size={14} /></button>
          ...
          <div className={`user-row ${frame.userFooterCard}`}>
            <Link href="/app/settings/profile" className={frame.userProfileLink}>
              {personalAvatar("user-avatar")}
              <span><strong>{userLabel}</strong><small>{roleLabel} · {tx("myProfile")}</small></span>
            </Link>
            <form action="/auth/signout" method="post" className={styles.signOutForm}>
              <button type="submit" className={styles.signOutButton} aria-label={tx("signOut")} title={tx("signOut")}><LogOut size={15} /></button>
            </form>
          </div>
        </aside>
        ...
        <section className="workspace">
          <header className="topbar">
            <div className="topbar-left"><button className="icon-btn mobile-only" onClick={() => setMobileNav(true)} aria-label={tx("openNavigation")}><Menu size={18} /></button></div>
            <div className="topbar-actions">
              <div className={frame.languageWrap}>
                <button type="button" className={frame.languageButton} onClick={() => setLanguageOpen(value => !value)} aria-expanded={languageOpen} aria-label={tx("language")}>
                  <span className={frame.languageFlag} aria-hidden="true">{locale === "fr" ? "🇫🇷" : "🇬🇧"}</span>
                  <span>{locale.toUpperCase()}</span>
                  <ChevronDown size={13} className={languageOpen ? frame.chevronOpen : ""} />
                </button>
                {languageOpen ? <div className={frame.languageMenu}>
                  <button type="button" className={locale === "en" ? frame.languageChoiceActive : ""} onClick={() => changeLocale("en")} disabled={switchingLocale}><span>🇬🇧</span><span><strong>English</strong><small>EN</small></span></button>
                  <button type="button" className={locale === "fr" ? frame.languageChoiceActive : ""} onClick={() => changeLocale("fr")} disabled={switchingLocale}><span>🇫🇷</span><span><strong>Français</strong><small>FR</small></span></button>
                </div> : null}
              </div>
              ...
              <div className={frame.notificationWrap}>
                <button className="icon-btn" type="button" aria-label={tx("notifications")} aria-expanded={notificationsOpen} onClick={() => setNotificationsOpen(value => !value)}><Bell size={17} />{attentionCount > 0 ? <span className="notification-dot" /> : null}</button>
                {notificationsOpen ? <div className={frame.notificationPanel}>
                  <div><strong>{tx("notifications")}</strong><button onClick={() => setNotificationsOpen(false)} aria-label={tx("closeNotifications")}><X size={14} /></button></div>
                  {attentionCount ? <Link href="/app/transactions" onClick={() => setNotificationsOpen(false)}><span className={frame.noticeIcon}><WalletCards size={15} /></span><span><strong>{locale === "fr" ? `${attentionCount} transaction${attentionCount === 1 ? "" : "s"} à vérifier` : `${attentionCount} transaction${attentionCount === 1 ? "" : "s"} need review`}</strong><small>{locale === "fr" ? `Ouvrir la vérification des transactions pour ${fiscalYear}` : `Open transaction review for ${fiscalYear}`}</small></span></Link> : <p>{locale === "fr" ? `Tout est à jour pour ${fiscalYear}.` : `You’re all caught up for ${fiscalYear}.`}</p>}
                  <Link href="/app/compliance" onClick={() => setNotificationsOpen(false)}><span className={frame.noticeIcon}><CalendarCheck2 size={15} /></span><span><strong>{tx("complianceCenter")}</strong><small>{locale === "fr" ? `Vérifier les obligations ${fiscalYear}` : `Review ${fiscalYear} obligations`}</small></span></Link>
                </div> : null}
              </div>
              <Link className={`icon-btn ${frame.profileButton}`} href="/app/settings/profile" aria-label={`${tx("myProfile")}: ${userLabel}`} title={tx("myProfile")}>
                {userAvatarUrl ? <span className={frame.topAvatar}><img src={userAvatarUrl} alt="" /></span> : <UserRound size={17} />}
              </Link>
            </div>
          </header>
          {children}
        ...
        {searchOpen ? <div className={frame.searchOverlay} role="dialog" aria-modal="true" aria-label={tx("searchZuelen")} onMouseDown={event => { if (event.currentTarget === event.target) setSearchOpen(false); }}>
          <div className={frame.searchDialog}>
            <div className={frame.searchInput}><Search size={18} /><input autoFocus value={query} onChange={event => setQuery(event.target.value)} placeholder={tx("searchPlaceholder")} /><button onClick={() => setSearchOpen(false)}><kbd>ESC</kbd></button></div>
            <div className={frame.searchResults}><p>{query ? tx("results") : tx("quickNavigation")}</p>{results.map(item => <Link href={item.href} key={item.href} onClick={() => setSearchOpen(false)}><span><item.icon size={17} /></span><div><strong>{item.label}</strong><small>{item.description}</small></div></Link>)}{results.length === 0 ? <div className={frame.noResults}>{tx("noMatchingWorkspace")}</div> : null}</div>
          </div>
        </div> : null}
      </main>
    </RoleProvider>
