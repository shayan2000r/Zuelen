...
export function InvoiceRecordActions({invoiceId,paymentStatus,status}:{invoiceId:string;paymentStatus:string;status:string}){
  const{canBookkeep}=useRolePermissions(),router=useRouter();const[deleteState,deleteAction,deletePending]=useActionState(voidInvoiceAction,initial);const[issueState,issueAction,issuePending]=useActionState(issueDraftInvoiceAction,initial);
