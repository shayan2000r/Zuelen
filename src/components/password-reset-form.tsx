...
export function PasswordResetForm({ locale }: { locale: "en" | "fr" }) {
  ...
  async function submit(event: FormEvent<HTMLFormElement>) {
    ...
    if (password.length < 8) {
      return setMessage(l("Use at least 8 characters.", "Utilisez au moins 8 caractères."));
    }
    if (password !== confirm) {
      return setMessage(l("The passwords do not match.", "Les mots de passe ne correspondent pas."));
    }
