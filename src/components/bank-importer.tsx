...
const aliases = {
  ...
  amount: ["amount","montant","montantoperation","montantdeloperation","betrag","value","transactionamount"],
  ...
  address: ["counterpartyaddress","address","adresse","adressedelacontrepartie"],
  communication: ["communication","remittanceinformation","purpose","memo","details","libelle","libellé"],
  operationCode: ["description","transactiontype","transactioncode","operationcode","operation_code","bookingcode"],
  reference: ["reference","paymentreference","referencepaiement"],
...
function cell(cells:string[],index:number){return index>=0?(cells[index]||"").trim():""}

function repairPostLuxembourgRow(cells:string[], headers:string[]) {
  if (cells.length <= headers.length) return cells;
  const normalizedHeaders=headers.map(norm);
  const communicationIndex=normalizedHeaders.indexOf("communication");
  const operationReferenceIndex=normalizedHeaders.findIndex(item=>["referencedoperation","referencedeloperation"].includes(item));
  if (communicationIndex < 0 || operationReferenceIndex !== headers.length - 1) return cells.slice(0,headers.length);
  const overflow=cells.length-headers.length;
  return [
    ...cells.slice(0,communicationIndex),
    cells.slice(communicationIndex, communicationIndex + overflow + 1).join(", "),
...
function normalizeCsv(text: string): { rows: NormalizedRow[]; message: string; headers: string[] } {
  ...
  for(const line of lines.slice(1)){
    const parsed=parseLine(line,sep),cells=repairPostLuxembourgRow(parsed,headers),date=parseDate(cells[dateI]||"");
    if(!date)continue;
    let amount=amountI>=0?parseNumber(cells[amountI]||""):0;
    if(amountI<0){const debit=debitI>=0?Math.abs(parseNumber(cells[debitI]||"")):0,credit=creditI>=0?Math.abs(parseNumber(cells[creditI]||"")):0; amount=credit-debit;}
    if(!Number.isFinite(amount)||amount===0)continue;
    const communication=cell(cells,communicationI),reference=cell(cells,referenceI),operationCode=cell(cells,operationCodeI);
    rows.push({
      booking_date:date,
      value_date:valueDateI>=0?parseDate(cells[valueDateI]||""):null,
      amount,
      currency:cell(cells,currencyI).toUpperCase()||undefined,
      counterparty_name:cell(cells,counterpartyI)||null,
      counterparty_iban:cell(cells,ibanI)||null,
      counterparty_address:cell(cells,addressI)||null,
      reference:reference||communication||null,
      communication:communication||null,
      operation_code:operationCode||null,
      external_id:cell(cells,externalI)||null,
    });
  }
  return{rows,message:rows.length?`${rows.length} valid transaction${rows.length===1?"":"s"} detected with merchant and bank-operation metadata.`:"No valid transactions were detected.",headers};
}
...
export function BankImporter({ defaultCurrency }: { defaultCurrency: string }) {
  ...
  return <>
    ...
    {open?<div className="compta-drawer-overlay" role="presentation" onMouseDown={e=>{if(e.currentTarget===e.target)setOpen(false)}}><div className="compta-drawer-shell"><button className="compta-drawer-close" type="button" onClick={()=>setOpen(false)} aria-label="Close bank import"><X size={18}/></button><aside className={`${styles.importCard} compta-drawer-card`}><div className={styles.importHead}><div><p>Statement intake</p><h2>Import bank CSV</h2></div><span><FileSpreadsheet size={14}/>CSV</span></div><p className={styles.lead}>Compta preserves merchant, communication and bank-operation metadata from common exports, including POST Luxembourg's French CSV format, so classification can use more than the visible merchant name.</p><form action={action} className={styles.importForm}>
      ...
      {preview.length>0?<div className={styles.preview}><div className={styles.previewHead}><span>Preview</span><strong>{rows.length} rows ready</strong></div>{preview.map((row,index)=><div className={styles.previewRow} key={`${row.booking_date}-${index}`}><span>{row.booking_date}</span><span>{row.counterparty_name||row.communication||row.operation_code||row.reference||"Bank movement"}</span><strong className={row.amount>=0?styles.moneyIn:styles.moneyOut}>{row.amount>=0?"+":"−"}{Math.abs(row.amount).toFixed(2)} {row.currency||defaultCurrency}</strong></div>)}</div>:null}
