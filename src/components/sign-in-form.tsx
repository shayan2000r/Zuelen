  ...
  async function handleSubmit(event: FormEvent<HTMLFormElement>) {
    ...
    try {
      ...
      if (mode === "signin") {
        ...
        const callback = `${window.location.origin}/auth/callback?next=/setup`;
        ...
        if (data.session) {
          ...
          setMessage("Check your inbox to confirm your email. The confirmation link will return you to Compta setup.");
        }
      }
    } catch (error) {
  ...
  return (
    <main className={styles.shell}>
      ...
      <section className={styles.formPanel}><div className={styles.formWrap}><div className={styles.formHeader}><span className={styles.formEyebrow}>{mode === "signin" ? "Welcome back" : "Create your workspace"}</span><h2>{mode === "signin" ? "Sign in to Compta" : "Start with your company"}</h2><p>{mode === "signin" ? "Continue where you left off." : "Set up your account now. Your Luxembourg company profile comes next."}</p></div>
        ...
        <div className={styles.switchMode}><span>{mode === "signin" ? "New to Compta?" : "Already have an account?"}</span><button type="button" onClick={() => { setMode(mode === "signin" ? "signup" : "signin"); setMessage(null); }}>{mode === "signin" ? "Create an account" : "Sign in"}</button></div>
      </div></section>
    </main>
  );
}
