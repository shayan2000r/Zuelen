 ...
 return <>
  ...
  {open?<div className={styles.drawerOverlay} role="presentation" onMouseDown={e=>{if(e.currentTarget===e.target)close()}}><div className={styles.drawerShell}><button autoFocus className={styles.drawerClose} type="button" onClick={close} aria-label={fr?"Fermer":"Close"}><X size={18}/></button><aside className={`${styles.formPanel} ${styles.drawerCard}`} role="dialog" aria-modal="true" aria-labelledby="new-transaction-title">
   {entryMode==="choose"?<><p className={styles.eyebrow}>{fr?"Nouvelle activité":"New activity"}</p><h2 id="new-transaction-title">{fr?"Ajouter une transaction":"Add a transaction"}</h2><p>{fr?"Choisissez la méthode la plus rapide. Vous pourrez vérifier les détails avant la comptabilisation.":"Choose the fastest way to start. You can review the details before anything is posted."}</p><div className={choiceStyles.choices}>
    <Link href="/app/documents?create=upload&purpose=transaction"><span><FileUp size={20}/></span><div><strong>{fr?"Importer un document":"Upload document"}</strong><small>{fr?"Ajoutez une facture ou un reçu. Zuelen peut ensuite extraire les montants, dates et informations visibles pour vérification.":"Add an invoice or receipt. Zuelen can then extract visible amounts, dates and details for review."}</small></div></Link>
    <Link href="/app/documents?create=scan&purpose=transaction"><span><Camera size={20}/></span><div><strong>{fr?"Scanner":"Scan"}</strong><small>{fr?"Ouvrez directement l’appareil photo sur mobile pour capturer un reçu ou une facture.":"Open the camera directly on mobile to capture a receipt or invoice."}</small></div></Link>
    <button type="button" onClick={()=>setEntryMode("manual")}><span><PenLine size={20}/></span><div><strong>{fr?"Saisie manuelle":"Manual"}</strong><small>{fr?"Saisissez vous-même le type, la date, le montant et le traitement TVA.":"Enter the type, date, amount and VAT treatment yourself."}</small></div></button>
