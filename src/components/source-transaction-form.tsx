...
export function SourceTransactionForm({defaultDate}:{defaultDate?:string}){
 const[state,formAction,pending]=useActionState(createSourceTransaction,initialTransactionState),[open,setOpen]=useState(false),[upgradeOpen,setUpgradeOpen]=useState(false),today=defaultDate||new Date().toISOString().slice(0,10);
 ...
 return <>
  <FloatingActionPortal><button className={styles.fab} type="button" onClick={()=>setOpen(true)}><Plus size={17}/>New transaction</button></FloatingActionPortal>
  {open?<div className={styles.drawerOverlay} role="presentation" onMouseDown={e=>{if(e.currentTarget===e.target)setOpen(false)}}><div className={styles.drawerShell}><button autoFocus className={styles.drawerClose} type="button" onClick={()=>setOpen(false)} aria-label="Close new transaction"><X size={18}/></button><aside className={`${styles.formPanel} ${styles.drawerCard}`} role="dialog" aria-modal="true" aria-labelledby="new-transaction-title">
   <p className={styles.eyebrow}>Record activity</p><h2 id="new-transaction-title">New transaction</h2><p>Tell Zuelen what happened. VAT treatment determines how the transaction is recorded in the ledger and reflected in filing evidence.</p>
