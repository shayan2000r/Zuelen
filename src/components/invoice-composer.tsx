...
import { ArrowLeft, BadgeCheck, FileCheck2, LoaderCircle, Plus, Send, Trash2 } from "lucide-react";
...
export function InvoiceComposer({ company }: { company: Company }) {
  const router = useRouter();
  const [state, formAction, pending] = useActionState(createAndIssueInvoice, initialInvoiceState);
  const [vatTreatment, setVatTreatment] = useState("domestic");
  ...
  const issuerCity = addressValue(company.registered_address, "city");
    ...
    <div className={styles.composerShell}>
      <form action={formAction} className={styles.composerForm}>
        ...
        <section className={styles.formSection}>
          ...
          <div className={styles.formGrid}>
            <label><span>Issue date</span><input name="issue_date" type="date" defaultValue={dateOffset(0)} required /></label>
            <label><span>Service date</span><input name="service_date" type="date" defaultValue={dateOffset(0)} required /></label>
            <label><span>Due date</span><input name="due_date" type="date" defaultValue={dateOffset(14)} required /></label>
            <label><span>VAT treatment</span><select name="vat_treatment" value={vatTreatment} onChange={(event) => setVatTreatment(event.target.value)}><option value="domestic">Luxembourg VAT</option><option value="eu_b2b_reverse_charge">EU B2B · reverse charge</option></select></label>
          </div>
