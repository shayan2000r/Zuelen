...
export function CompanySetup() {
  ...
  async function handleSubmit(event: FormEvent<HTMLFormElement>) {
    ...
    try {
      ...
      const slug = `${slugBase}-${authData.user.id.slice(0, 6)}`;
      const { data: organization, error: orgError } = await supabase
        .from("organizations")
        .insert({ name: legalName, slug, owner_id: authData.user.id })
        .select("id")
        .single();
      ...
      const { error: companyError } = await supabase.from("companies").insert({
        ...
        vat_filing_frequency: vatRegistered ? "annual" : null,
        base_currency: "EUR",
      });
  ...
  return (
    <main className={styles.setupShell}>
      <header className={styles.setupTop}>
        <div className={styles.setupBrand}><div className={styles.logoMark}>C</div><span>Compta</span></div>
        <span className={styles.secureTag}><ShieldCheck size={13} />Secure workspace setup</span>
      </header>
      <div className={styles.setupGrid}>
        <aside className={styles.setupAside}>
          <p className={styles.overline}>Company 01</p>
          <h1>Tell Compta who you are.</h1>
          <p>We use this profile to decide which accounting and compliance workflows apply to your company.</p>
          <div className={styles.setupSteps}>
            <div className={styles.activeStep}><span>01</span><p><strong>Company profile</strong><small>Legal identity and VAT status</small></p></div>
            <div><span>02</span><p><strong>Opening position</strong><small>Bank, accounting year and balances</small></p></div>
            <div><span>03</span><p><strong>Compliance map</strong><small>Your obligations and next deadlines</small></p></div>
          </div>
        </aside>

        <section className={styles.setupCard}>
          <div className={styles.setupIcon}><Building2 size={21} /></div>
          <h2>Company profile</h2>
          <p className={styles.cardLead}>Start with the official information you already use on invoices and government portals.</p>
          <form className={styles.setupForm} onSubmit={handleSubmit}>
            ...
            <div className={`${styles.formActions} ${styles.fullField}`}>
              <button type="button" className={styles.backButton} onClick={() => router.push("/sign-in")}><ChevronLeft size={15} />Back</button>
              <button type="submit" className={styles.submit} disabled={loading}>{loading ? <LoaderCircle className={styles.spin} size={17} /> : <Check size={16} />}<span>Create workspace</span>{!loading ? <ArrowRight size={16} /> : null}</button>
            </div>
          </form>
        </section>
      </div>
